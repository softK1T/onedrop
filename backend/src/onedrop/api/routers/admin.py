...
def require_admin(user: CurrentUser) -> User:
    allowed = set(get_settings().admin_telegram_user_ids)
    if user.telegram_user_id not in allowed:
...
async def overview(session: DbSession, _admin: AdminUser) -> AdminOverview:
    ...
    capture_rows = (await session.execute(select(InboxItem.status, func.count()).group_by(InboxItem.status))).all()
    captures = {str(status): int(count) for status, count in capture_rows}
    notification_rows = (await session.execute(select(ScheduledNotification.status, func.count()).group_by(ScheduledNotification.status))).all()
    notifications = {str(status): int(count) for status, count in notification_rows}
    ai_count, ai_cost = (await session.execute(select(func.count(AiOperation.id), func.coalesce(func.sum(AiOperation.cost_micro), 0)))).one()
    provider_rows = (await session.execute(select(AiOperation.provider, func.coalesce(func.sum(AiOperation.cost_micro), 0)).group_by(AiOperation.provider))).all()
    active_subscriptions = int(await session.scalar(select(func.count()).select_from(Subscription).where(Subscription.status == "active", (Subscription.expires_at.is_(None)) | (Subscription.expires_at > now))) or 0)
    payment_count, payment_total = (await session.execute(select(func.count(Payment.id), func.coalesce(func.sum(Payment.amount_stars), 0)).where(Payment.status == "paid"))).one()
