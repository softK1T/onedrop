...
from onedrop.api.routers import admin as admin_router
...
from onedrop.api.routers import habits as habits_router
...
from onedrop.api.routers import users as users_router
...
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    get_logger(__name__).info("api.startup", environment=settings.app_env, ai_provider_mode=settings.ai_provider_mode, telegram_enabled=settings.telegram_enabled)
    yield
...
def build_api_router() -> APIRouter:
    ...
    for module in (system_router, auth_router, users_router, capture_router, inbox_router, tasks_router, events_router, expenses_router, meals_router, habits_router, notes_router, analytics_router, billing_router, telegram_router, admin_router):
...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(title=f"{settings.app_name} API", version=__version__, summary="AI-powered Telegram life planner", docs_url=None if settings.is_production else "/docs", redoc_url=None, openapi_url=None if settings.is_production else "/openapi.json", lifespan=lifespan)
    app.add_middleware(TrustedHostMiddleware, allowed_hosts=settings.trusted_host_list)
    app.add_middleware(CORSMiddleware, allow_origins=settings.cors_origin_list, allow_credentials=True, allow_methods=["GET", "POST", "PATCH", "DELETE", "OPTIONS"], allow_headers=["Authorization", "Content-Type", "X-Request-ID"], max_age=600)
