...
@dataclass(frozen=True, slots=True)
class DispatcherRuntimeOptions:
    """Settings-derived operational limits for one dispatcher instance."""
...
def runtime_options(settings: Settings) -> DispatcherRuntimeOptions:
    """Map validated application settings to the dispatcher runtime policy."""
    return DispatcherRuntimeOptions(
        policy=RetryPolicy(
            max_attempts=settings.reminder_max_attempts,
            base_delay_seconds=settings.reminder_base_delay_seconds,
            max_delay_seconds=settings.reminder_max_delay_seconds,
        ),
        claim_limit=settings.reminder_claim_limit,
        lock_timeout=timedelta(seconds=settings.reminder_lock_timeout_seconds),
    )
...
class NotificationDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._policy = policy or options.policy
        self._worker_id = worker_id or default_worker_id()
        self._claim_limit = claim_limit or options.claim_limit
